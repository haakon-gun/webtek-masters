document.write(`
<footer id="website-footer">
    <section id="map-container" class="website-footer-section">
        <iframe id="map" src="https://www.google.com/maps/embed/v1/place?q=place_id:ChIJWdJJ6SwhR0YRa2D-z285A2E&key=AIzaSyBITdjEW51MyztEZQGDNFcxuz1YZP_KUlk" allowfullscreen></iframe>
    </section>

    <section class="website-footer-list website-footer-section">
        <div class="website-footer-horizontal-stack">
            <h1 class="website-footer-heading">Adresse</h1>
            <p>Henrik Ibsens gate 6: 3. etasje. 3724 Skien</p>
            <a href="tel:+4735520469">Telefon: 35 52 04 69</a>
        </div>
        <div class="website-footer-horizontal-stack">
            <h1 class="website-footer-heading">Åpningstider</h1>
            <p>08:00–15:00, mandag til torsdag</p>
            <p>08:00–13:00, fredag</p>
        </div>
        <div class="website-footer-horizontal-stack">
            <h1 class="website-footer-heading">Grenland tannlegevakt</h1>
            <p>Helger og helligdager: 10:00–17:00</p>
            <a href="tel:+4790913377">Telefon: 90 91 33 77</a>
        </div>

        <div class="website-footer-horizontal-stack">
            <a href="https://www.tannlegeforeningen.no">
                <img id="logo" alt="Den norske tannlegeforeningen" src="resources/dntf.png">
                <h1 class="website-footer-heading">Den norske tannlegeforeningen</h1>
            </a>
        </div>
    </section>
</footer>
`);
